...
export async function validateBalances(db: MyDatabase, tonClient: TonClient, iotaClient: Client) {
    try {
        ...
        // Prices
        const nftOutputString = await iotaClient.nftOutputId(NFT_ID);
        const output = await iotaClient.getOutput(nftOutputString);
        const nftOutput = output.output as unknown as NftOutput;
        const meta = nftOutput.getFeatures()[0] as MetadataFeature;
        const prices = JSON.parse(hexToUtf8(meta.data)) as Prices;
        const pricesCell = Cell.fromBoc(Buffer.from(prices.packedPrices, 'hex'))[0]
        const pricesCellBase64 = pricesCell.toBoc().toString('base64');
        let pricesDict = pricesCell.beginParse()
        ...
        for (const user of users) {
            ...
            if (totalLimit < totalDebt) {
                ...
                if (gLoanAsset === AssetID.ton) {
                    ...
                    values.push(gLoanValue * decimals.jetton / gLoanAssetPrice);
                }

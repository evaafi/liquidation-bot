...
export class MyDatabase {
    ...
    async updateUser(contract_address: string, code_version: number, created_at: number, updated_at,
        ...
        await this.db.run(`
            UPDATE users 
            SET code_version = ?, created_at = ?, updated_at = ?, ton_principal = ?, usdt_principal = ?, usdc_principal = ?
            WHERE contract_address = ?
        `, code_version, created_at, updated_at, tonPrincipal.toString(), usdtPrincipal.toString(), usdcPrincipal.toString(), contract_address)
    }
    ...
    async handleFailedTasks() {
        await this.db.run(`
            ...
            WHERE state = 'sent' AND ? - updated_at > 30000
        `, Date.now(), Date.now())
    ...
    async isTaskExists(walletAddress: string) {
        const result = await this.db.get(`
            ...
            WHERE 
                ...
                (wallet_address = ? AND state = 'sent' AND ? - updated_at < 45000) OR
